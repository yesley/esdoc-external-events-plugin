...
class Plugin {
    onHandleConfig (ev) {
        ...
        if (!('enable' in this._option)) {
            this._option.enable = true;
        }

        if (!this._option.enable) {
            return;
        }

        const srcPath = path.resolve(__dirname, 'external-events.js');
        const outPath = path.resolve(this._config.source, '.external-events.js');
    ...
    onHandleDocs (ev) {
        ...
        const name = path.basename(path.resolve(this._config.source)) + '/.external-events.js';
